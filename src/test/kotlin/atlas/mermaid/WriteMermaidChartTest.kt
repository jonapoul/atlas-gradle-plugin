...
internal class WriteMermaidChartTest : ScenarioTest() {
  ...
  @Test
  fun `Write chart with groups enabled and projects nested`() = MermaidWithGroupsNested {
    // when
    assertThatTask(":a:writeMermaidChart").buildsSuccessfully().allTasksSuccessful()
  ...
  @Test
  fun `Write chart configured by gradle properties`() = MermaidConfiguredByProperties {
    // when
    assertThatTask(":a:writeMermaidChart").buildsSuccessfully().allTasksSuccessful()
