...
internal class WriteD2ChartTest : ScenarioTest() {
  @Test
  fun `Generate charts from basic config`() = D2Basic {
    // when
  ...
  @Test
  @RequiresD2
  fun `Write correct classes file path for nested projects`() = D2NestedProjects {
    // when
  ...
  @Test
  fun `Write correct classes file path when outputs are moved`() = D2MovedOutputs {
    // when
    assertThatTask("writeD2Chart").buildsSuccessfully().allTasksSuccessful()
