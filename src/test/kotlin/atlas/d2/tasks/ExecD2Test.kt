...
internal class ExecD2Test : ScenarioTest() {
  @Test
  fun `No extras are generated if no file formats have been declared`() = D2Basic {
  ...
  @Test
  @RequiresD2
  fun `Choose custom layout engine`() = D2CustomLayoutEngine {
  ...
  @Test
  @RequiresD2
  fun `Rerun when changing properties in the classes file`() = D2CustomLayoutEngine {
  ...
  @Test
  fun `Pass extra CLI flags to D2`() = D2CliFlags {
  ...
  @Test fun `Download d2 instead of using the PATH`() = D2Basic { assertD2Downloaded() }

  @Test
  fun `Download d2 when its version is set`() = D2PinnedVersion {
    assertD2Downloaded(source = "auto")
  }

  @Test
  fun `Download d2 when settings repositories are preferred`() = GroovyD2PreferSettings {
    assertD2Downloaded()
  }

  @Test
  @RequiresD2
  fun `Use d2 from the PATH by default`() = D2Basic {
    assertThatTask(":a:execD2Chart")
      .withArgument("--info")
      .buildsSuccessfully()
  ...
  @Test
  fun `Download d2 when projects declare their own repositories`() = D2WithProjectRepositories {
    assertD2Downloaded()
  }

  @Test
  fun `Download d2 when project repositories are forbidden`() = D2FailOnProjectRepos {
    assertD2Downloaded()
  }
